...
const static int pos_zero[MAX_NUM_DEV] = {500, 500, 500, 500, 500, 500, 500, 500};
const static int pos_dest[MAX_NUM_DEV] = {20000, 20000, 20000, 20000, 20000, 20000, 20000, 20000};
const static int ampr_zero[MAX_NUM_DEV] = {100, 100, 100, 100, 100, 100, 100, 100};
const static int ampr_dest[MAX_NUM_DEV] = {100, 100, 100, 100, 100, 100, 100, 100};
...
const static int plan_vel_low[MAX_NUM_DEV] = {50, 50, 50, 50, 50, 50, 50, 50};
...
void t_sdfb(void) /* Task: ShielD of Front/Back */
{
        ...
        for (;;) {
                ...
                case sizeof(struct main):
                        ...
                        switch (verify.type) {
                        case CMD_IDLE:
                        case CMD_ACT_SDF | CMD_MODE_AUTO | CMD_DIR_STOP:
                        case CMD_ACT_SDF | CMD_MODE_MANUAL | CMD_DIR_STOP:
                        case CMD_ACT_SDB | CMD_MODE_AUTO | CMD_DIR_STOP:
                        case CMD_ACT_SDB | CMD_MODE_MANUAL | CMD_DIR_STOP:
                                switch (cmd.type) {
                                case CMD_IDLE:
                                case CMD_ACT_SDF | CMD_MODE_AUTO | CMD_DIR_POSI:
                                case CMD_ACT_SDF | CMD_MODE_AUTO | CMD_DIR_NEGA:
                                case CMD_ACT_SDF | CMD_MODE_AUTO | CMD_DIR_STOP:
                                case CMD_ACT_SDF | CMD_MODE_MANUAL | CMD_DIR_POSI:
                                case CMD_ACT_SDF | CMD_MODE_MANUAL | CMD_DIR_NEGA:
                                case CMD_ACT_SDF | CMD_MODE_MANUAL | CMD_DIR_STOP:
                                case CMD_ACT_SDB | CMD_MODE_AUTO | CMD_DIR_POSI:
                                case CMD_ACT_SDB | CMD_MODE_AUTO | CMD_DIR_NEGA:
                                case CMD_ACT_SDB | CMD_MODE_AUTO | CMD_DIR_STOP:
                                case CMD_ACT_SDB | CMD_MODE_MANUAL | CMD_DIR_POSI:
                                case CMD_ACT_SDB | CMD_MODE_MANUAL | CMD_DIR_NEGA:
                                case CMD_ACT_SDB | CMD_MODE_MANUAL | CMD_DIR_STOP:
                                        verify = cmd;
                                        break;
                                default:
                ...
                case sizeof(struct frame_can):
                        ...
                        all_zero = result[0] & result[1] & result[2] & result[3] & result[4] & result[5] & result[6] & result[7] & RESULT_ZERO;
                        all_dest = result[0] & result[1] & result[2] & result[3] & result[4] & result[5] & result[6] & result[7] & RESULT_DEST;
                        ...
                        if (tmp_sync == -1) {
                                ...
                                result[4] |= RESULT_FAULT_SYNC;
                                result[5] |= RESULT_FAULT_SYNC;
                                result[6] |= RESULT_FAULT_SYNC;
                                result[7] |= RESULT_FAULT_SYNC;
                        } else if (tmp_sync == 1) {
                                result[0] &= ~RESULT_FAULT_SYNC;
                                result[1] &= ~RESULT_FAULT_SYNC;
                                result[2] &= ~RESULT_FAULT_SYNC;
                ...
                default:
                        ...
                        if ((verify.type & UNMASK_CMD_MODE) == CMD_MODE_AUTO)
                                any_fault = (result[0] | result[1] | result[2] | result[3] | result[4] | result[5] | result[6] | result[7]) & UNMASK_RESULT_FAULT;
                        else if ((verify.type & UNMASK_CMD_MODE) == CMD_MODE_MANUAL)
                                any_fault = (result[0] | result[1] | result[2] | result[3] | result[4] | result[5] | result[6] | result[7]) & UNMASK_RESULT_FAULT & ~RESULT_FAULT_SYNC;
                        if (any_fault) {

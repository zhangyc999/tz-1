...
const static int io_pos_dest[MAX_NUM_DEV] = {10000, 10000, 10000, 10000};
const static int min_pos[MAX_NUM_DEV] = {-1000, -1000, -1000, -1000};
const static int max_pos[MAX_NUM_DEV] = {20000, 20000, 20000, 20000}; /* 400 */
...
void t_y(void) /* Task: crane on the front for Y-axis */
{
        ...
        for (;;) {
                ...
                case sizeof(struct main):
                        ...
                        if ((cmd.type & UNMASK_TASK_NOTIFY) == TASK_NOTIFY_VSL) {
                                if ((cmd.type & UNMASK_TASK_STATE) == TASK_STATE_RUNNING) {
                                        ...
                                        delta[1] = (s16)((cmd.data & 0xFF00) >> 16);
                                        delta[2] = (s16)(cmd.data & 0x00FF);
                                        delta[3] = (s16)(cmd.data & 0x00FF);
                                } else {
                                        delta[0] = 0;
                                        delta[1] = 0;
                                        delta[2] = 0;
                ...
                case sizeof(struct frame_can):
                        ...
                        if (tmp_sync_f == -1 || tmp_sync == -1) {
                                result[0] |= RESULT_FAULT_SYNC;
                                result[1] |= RESULT_FAULT_SYNC;
                        } else if (tmp_sync_f == 1 && tmp_sync == 1) {
                                result[0] &= ~RESULT_FAULT_SYNC;
                                result[1] &= ~RESULT_FAULT_SYNC;
                        }
                        if (tmp_sync_b == -1 || tmp_sync == -1) {
                                result[2] |= RESULT_FAULT_SYNC;
                                result[3] |= RESULT_FAULT_SYNC;
                        } else if (tmp_sync_b == 1 && tmp_sync == 1) {
                                result[2] &= ~RESULT_FAULT_SYNC;
                                result[3] &= ~RESULT_FAULT_SYNC;
                        }
                        period -= tickGet() - prev;
